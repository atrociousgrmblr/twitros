...
class TwitterServer:
            ...
            if 'media' in s['entities']:
                for media in s['entities']['media']:
                    if media['type'] == 'photo':
                        ...
                        try:
                            tweet.image = self.bridge.cv_to_imgmsg(
    ...
    def timer_home_cb(self, event):
        ...
        if not remaining:
            ...
            period = rospy.Duration.from_sec( 
                    ( reset - rospy.Time.now().secs ) / remaining )
            timer_home = rospy.Timer( 
    ...
    def timer_mentions_cb(self, event):
        ...
        if not remaining:
            ...
            rospy.sleep( rospy.Duration( reset - rospy.Time.now().secs ) )
            timer_mentions = rospy.Timer( 
                    rospy.Duration(1), self.timer_mentions_cb, oneshot = True )
        else:
            period = rospy.Duration.from_sec( 
                    ( reset - rospy.Time.now().secs ) / remaining )
            timer_mentions = rospy.Timer( 
                    period, self.timer_mentions_cb, oneshot = True )

    def timer_dm_cb(self, event):
        # Direct messages (make sure to not skip status)
        response = self.t.getDirectMessages( 
                since_id = self.last_dm, skip_status = False )
        ...
        if not remaining:
            ...
            rospy.sleep( rospy.Duration( reset - rospy.Time.now().secs ) )
            timer_dm = rospy.Timer( 
                    rospy.Duration(1), self.timer_dm_cb, oneshot = True)
